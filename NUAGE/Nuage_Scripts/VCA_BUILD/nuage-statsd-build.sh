...
# COMPRESSED SSL
./configure --enable-Werror --with-vendor=vrs --prefix=/usr --sysconfdir=/etc --localstatedir=/var  CPPFLAGS="-DSTATSD_UT -DSTATSD_BUFFER_COMPRESS"  CFLAGS="-g -O0 `xml2-config --cflags`" LIBS="-lrt -lm `xml2-config --libs` -lpthread -lanl" && make
...
cd ovs
# UNCOMPRESSED SSL
./configure --enable-Werror --with-vendor=vrs --prefix=/usr --sysconfdir=/etc --localstatedir=/var  CPPFLAGS="-DSTATSD_UT"  CFLAGS="-g -O0 `xml2-config --cflags`" LIBS="-lrt -lm `xml2-config --libs` -lpthread -lanl" && make
